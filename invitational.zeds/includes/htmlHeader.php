...
<tr><td>Name:</td><td style="text-align: center; width: 75px; border: 0px;"><input id="fname" name="fname" style="width: 75px;" onkeyup="updateOne();"></td><td style="text-align: center; width: 75px; border: 0px;"><input id="lname" name="lname" style="width: 75px;" onkeyup="updateOne();"></td></tr>\
...
$query = mysql_query("SELECT * FROM `schools` WHERE `SID`!='1' ORDER BY `name`;");
...
<tr><td>Will you<br>need housing?</td><td colspan="2"><label for="yes"><input type="radio" name="housing" id="yes" value="yes" onchange="updateOne();">Yes</label><br><label for="no"><input type="radio" name="housing" id="no" value="no" onchange="updateOne();">No</label></td></tr>\
</table><table>\
...
function continueOne(){
	...
	var unique = getFile("http://colacadstink.getmyip.com:800/invitational/api/uniqueUserEmailCheck.php?email="+document.getElementById("email").value+"&user="+document.getElementById("username").value);
	if(unique!="OK"){
	...
	var isHousing = document.getElementById("yes").checked;
...
';
	...
	if(isAlreadySigned){
		var waiverStatus = getFile("http://colacadstink.getmyip.com:800/api/hasEmailTurnedInWaiver.php?email="+document.getElementById("email").value);
		if(waiverStatus=="OK"){
	...
	if(isHousing){
		newInnerHTML += '<br>I will need housing:<br><label for="short"><input type="radio" id="short" name="housingDuration" value="short" onchange="'updateTwo();"> the night of the 26th, leaving the night of the 27th</label><br>\
<label for="short2"><input type="radio" id="short2" name="housingDuration" value="short2" onchange="'updateTwo();"> the night of the 27th, leaving the morning of the 28th</label><br>\
<label for="long"><input type="radio" id="long" name="housingDuration" value="long" onchange="'updateTwo();"> the nights of the 26th and 27th, leaving the morning of the 28th</label><br><br>';
		newInnerHTML += 'If you would like to request roommate(s),<br>please enter their names here:<br><textarea name="roommateRequest" onkeyup="'updateTwo();"></textarea><br>\n';
	}
...
function updateTwo(){
	if(document.getElementById("short") != null && !document.getElementById("short").checked && !document.getElementById("long").checked) return disableContinue();
...
Thank you for registering with Z.E.D.S., we look forward to seeing you on April 27th!<br>\
