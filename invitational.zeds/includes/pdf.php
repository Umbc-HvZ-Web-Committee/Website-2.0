...
function createID($pdf, $row, $col, $userInfo){
	...
	//clearance level
	$fg = colorNameToForeground($userInfo['clearanceLevel']);
	$bg = colorNameToBackground($userInfo['clearanceLevel']);
	$pdf->SetTextColor($fg[0], $fg[1], $fg[2]);
	$pdf->setFillColor($bg[0], $bg[1], $bg[2]);
	$pdf->SetFont('helvetica', 'B', 13);
	$pdf->MultiCell(inches(1.35), inches(0.5), "Clearance level:\n".$userInfo['clearanceLevel'], 0, 'L', true, 0, inches(.25+($col*$COL_SPACING)+.1), inches(.25+($row*2.35)+1.25), true, 0, false, true, inches(2.25), 'T', false);
